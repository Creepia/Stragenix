...
def getIndicators(df):
    ...
    df["WPR"] = atb.get_WPR(df["High"], df["Low"], df["Close"], 14)
    ...
    df=atb.get_KD(df,window=14,k=3,d=2)
...
def RUN_FOLDER(folder,get_indicator_func=getIndicators,initial_cash=1000000,output_folder="Conclusion"):
...
def index():
    ...
    return render_template("index.html", folder="data", output_folder="Conclusion", initial_cash="1000000", get_indicator_func=getIndicators.__code__, get_signal_func=atb.deci_func.__code__)
